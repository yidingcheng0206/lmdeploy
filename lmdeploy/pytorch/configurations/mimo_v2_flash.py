...
def _finalize_mimo_cache_specs(model_config: ModelConfig, block_size: int, *, tp: int):
    ...
    if getattr(hf_config, 'mimo_target_swa_paged', False):
        block_specs.extend(
            [
                BlockCacheSpec(
                    'mimo_swa_paged_k',
                    swa_layers,
                    (block_size, swa_heads, hf_config.swa_head_dim),
                    model_config.dtype,
                ),
                BlockCacheSpec(
                    'mimo_swa_paged_v',
                    swa_layers,
                    (block_size, swa_heads, hf_config.swa_v_head_dim),
                    model_config.dtype,
                ),
            ]
        )
        model_config.block_cache_specs = block_specs
        model_config.state_cache_specs = []
...
class MiMoV2FlashModelConfigBuilder(AutoModelConfigBuilder):
    ...
    def build(
        ...
        **kwargs,
        ...
        """Build target or three-depth draft configuration and cache specs."""
        ...
        hf_config.mimo_target_swa_paged = spec_method in supported_spec_methods
        num_key_value_heads = _update_kv_heads_for_tp(
            hf_config,
