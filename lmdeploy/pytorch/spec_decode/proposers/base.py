...
class BaseSpecProposer:
    ...
    def get_next_depth_dp_num_tokens(self, dp_meta) -> list[int]:
        """Return per-rank token counts after advancing a draft depth.

        Recurrent draft models compact the next depth to one token per active
        request, so the DP batch counts are also the token counts. Draft
        architectures that retain a varlen sequence across depths may
