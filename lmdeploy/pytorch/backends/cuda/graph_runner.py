...
def _support_spec_decode_without_fa3(model: torch.nn.Module) -> bool:
    """Resolve the model capability for non-FA3 speculative Graphs."""
    capability = getattr(model, 'support_spec_decode_without_fa3', None)
...
def _requires_fa3_for_spec_decode(model: torch.nn.Module, model_config: ModelConfig) -> bool:
    ...
    return (
        ...
        and not _support_spec_decode_without_fa3(model)
    )
