...
def fa3_build_supports_head_dims(head_size: int, v_head_size: int | None = None) -> bool:
    ...
    FA3 wheels may omit template instantiations to reduce build time and binary
    size. Wheels without exported build metadata are accepted only for
    symmetric head shapes, preserving compatibility with older packages while
    avoiding unsafe asymmetric dispatch.
    """
